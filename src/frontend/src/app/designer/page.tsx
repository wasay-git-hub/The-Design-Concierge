...
import { 
  ...
  User, Mail, Phone, MapPin, Gauge, Download, Home, Maximize2, Loader2, MessageSquare
} from "lucide-react";
...
export default function DesignerDashboard() {
  const [activeTab, setActiveTab] = useState<"leads" | "feedback">("leads");
  ...
  const fetchFeedbacks = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Could not retrieve feedback from the database.");
      const data = await res.json();
      setFeedbacks(data);
    } catch (err: any) {
  ...
  const refreshAll = async () => {
    setRefreshing(true);
    setErrorMsg("");
    await Promise.all([fetchLeads(), fetchFeedbacks()]);
    setRefreshing(false);
  };
    ...
    <div className="flex flex-col min-h-screen bg-luxury-lightGray/40">
      ...
      <main className="flex-1 flex flex-col md:flex-row max-w-7xl mx-auto w-full p-4 md:p-8 gap-6">
          ...
          <div className="p-4 border-b border-luxury-border flex justify-between items-center bg-luxury-cream/30">
            <div className="flex gap-6">
              <button 
                ...
                className={`font-serif text-lg font-semibold transition pb-1 ${activeTab === "leads" ? "text-luxury-charcoal border-b-2 border-luxury-brass" : "text-luxury-charcoal/40 hover:text-luxury-charcoal/70"}`}
              >
                Leads
              </button>
              <button 
                onClick={() => setActiveTab("feedback")}
                className={`font-serif text-lg font-semibold transition pb-1 ${activeTab === "feedback" ? "text-luxury-charcoal border-b-2 border-luxury-brass" : "text-luxury-charcoal/40 hover:text-luxury-charcoal/70"}`}
              >
                Feedback
              </button>
            </div>
            <span className="text-xs text-luxury-charcoal/60 font-semibold">
              {activeTab === "leads" ? `${leads.length} Leads` : `${feedbacks.length} Msgs`}
            </span>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-luxury-border max-h-[600px]">
              ...
              {activeTab === "leads" ? (
                ...
                feedbacks.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
                    <MessageSquare className="w-12 h-12 text-luxury-border mb-3" />
                  ...
                  feedbacks.map((item) => (
                    <div 
                      ...
                      className={`p-4 cursor-pointer hover:bg-luxury-lightGray/30 transition flex flex-col gap-2 ${
                        selectedFeedback?.id === item.id ? "bg-luxury-cream/60 border-l-4 border-l-luxury-brass" : ""
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        <span className="font-semibold text-sm text-luxury-charcoal">{item.name || "Anonymous User"}</span>
                        <span className="text-[10px] text-luxury-charcoal/40">{new Date(item.created_at).toLocaleDateString()}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold px-2 py-0.5 bg-luxury-brass/10 text-luxury-brass rounded border border-luxury-brass/30 whitespace-nowrap">
                          {item.category}
                        </span>
                        <p className="text-xs text-luxury-charcoal/70 truncate w-40">{item.message}</p>
                      </div>
                    </div>
                  ))
                )
        ...
        <div className="flex-1 bg-white border border-luxury-border rounded-xl shadow-sm overflow-hidden flex flex-col">
          {activeTab === "feedback" ? (
            selectedFeedback ? (
              <div className="flex-1 flex flex-col p-8 bg-luxury-cream/10">
                <div className="flex items-center gap-2 text-luxury-brass mb-6">
                  <MessageSquare className="w-6 h-6" />
                  <span className="font-bold text-xs uppercase tracking-widest">{selectedFeedback.category} Report</span>
                </div>
                <h2 className="font-serif text-3xl font-semibold text-luxury-charcoal mb-2">{selectedFeedback.name || "Anonymous User"}</h2>
                <p className="text-sm text-luxury-charcoal/60 mb-8">{selectedFeedback.email || "No email provided"} • {new Date(selectedFeedback.created_at).toLocaleString()}</p>
                <div className="bg-white border border-luxury-border rounded-xl p-8 shadow-sm">
                  <p className="text-luxury-charcoal/90 leading-relaxed whitespace-pre-wrap">{selectedFeedback.message}</p>
                </div>
              </div>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
                <MessageSquare className="w-16 h-16 text-luxury-border mb-4 animate-pulse" />
                <h3 className="font-serif text-xl font-semibold text-luxury-charcoal">Select a Feedback Message</h3>
                <p className="text-xs text-luxury-charcoal/50 mt-2">Click on any client feedback on the left to review the details.</p>
              </div>
            )
          ) : (
            selectedLead ? (
